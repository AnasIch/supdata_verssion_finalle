...
const empty = { nom:"", detail:"", agence:"Casablanca", quantite:"", type:"Entrée", document_type:"", document_file:null, fournisseur:"", bon_livraison:"" };
const warning = ["Stock faible","À valider","À confirmer","En cours","En préparation"];
...
export default function StockOperationsPanel({ operations }) {
 ...
 const submit=()=>{const isCategory=operations.section==="categories";if(!form.nom||(!isCategory&&(form.quantite===""||Number(form.quantite)<0)))return toast("Vérifiez les informations saisies.","error");const needsDocument=operations.section==="mouvements";if(needsDocument&&!form.document_type)return toast("Renseignez un type de document.","error");if(needsDocument&&!form.document_file)return toast("Joignez le document PDF associé.","error"); editing?operations.updateItem(editing.id,form):operations.createItem(form);setFormOpen(false);toast(editing?"Élément modifié.":"Élément créé.")};
 const transition=i=>{operations.transitionItem(i.id);toast(operations.section==="livraisons"?"Remise au client confirmée.":"Alerte traitée.")};
 ...
 const actions=i=><div className="flex justify-end gap-2">
  ...
  {operations.config.workflow&&operations.canTransition(i)&&<Button size="sm" onClick={()=>transition(i)}>{operations.section==="livraisons"?"Livrer":"Traiter"}</Button>}
