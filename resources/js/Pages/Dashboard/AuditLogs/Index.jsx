...
const auditStatuses = {
    Succès: "bg-emerald-50 text-emerald-600 border-emerald-200",
...
export default function AuditLogsIndex() {
    ...
    const auditModules = useMemo(() => ["Tous", ...(serverModules || [])], [serverModules]);
    const auditActions = useMemo(() => ["Toutes", ...(serverActions || [])], [serverActions]);

    const [selectedLog, setSelectedLog] = useState(null);

    const kpis = useMemo(() => [
        { icon: Activity, label: "Total actions", value: String(stats.total), trend: `+${stats.today}`, trendUp: true, description: "toutes périodes", color: "bg-blue-50 text-blue-600" },
        { icon: Clock, label: "Aujourd'hui", value: String(stats.today), trend: "+5", trendUp: true, description: "actions aujourd'hui", color: "bg-emerald-50 text-emerald-600" },
        { icon: Shield, label: "Succès", value: String(stats.success), trend: stats.total > 0 ? `${Math.round((stats.success / stats.total) * 100)}%` : "0%", trendUp: true, description: "taux de réussite", color: "bg-violet-50 text-violet-600" },
        { icon: AlertTriangle, label: "Échoués", value: String(stats.failed), trend: stats.failed > 0 ? `${stats.failed}` : "0", trendUp: false, description: "nécessitent attention", color: "bg-amber-50 text-amber-600" },
    ], [stats]);
